...
def main():
    ...
    parser.add_argument('price', type=float, help='Price at which to place the limit order')
    ...
    result = bot.place_limit_order(args.symbol, args.quantity, args.price, args.side)

    if result["status"] == "success":
        print(f"Limit order placed successfully! Order ID: {result['order']['orderId']}")
        print(f"Details: {result['order']}")
    else:
        print(f"Failed to place limit order: {result['message']}")
